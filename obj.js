...
class ObjMesh
{
	...
	shiftAndScale( shift, scale )
	{
		for ( var i=0; i<this.vpos.length; ++i ) {
			for ( var j=0; j<3; ++j ) {
				// console.log("this.vpos[i][j] + shift[j]) * scale", this.vpos[i][j] ,shift[j] , scale);
				this.vpos[i][j] = (this.vpos[i][j] + shift[j]) * scale;
				// console.log("after ", this.vpos[i][j]);
			}
		}
		// console.log("log from obj.s shift and scale () --- this.vpos",this.vpos);
	}
	rotate(angle, axis) {
		// Create the rotation matrix based on the angle and axis
		...
		// Apply the rotation matrix to each vertex position
		for (let i = 0; i < this.vpos.length; i++) {
		  const newVertex = [];
		  for (let j = 0; j < 3; j++) {
			let sum = 0;
			for (let k = 0; k < 3; k++) {
			  sum += rotMatrix[j][k] * this.vpos[i][k];
			}
			newVertex.push(sum);
		  }
		  this.vpos[i] = newVertex;
		}
	  }
